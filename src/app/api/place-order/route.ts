...
async function createWooOrder(base: string, auth: string, payload: any) {
  const resp = await fetch(`${base}/wp-json/wc/v3/orders`, {
    method: "POST",
    headers: {
      Authorization: auth,
...
export async function POST(req: Request) {
  try {
    ...
    const { ok, status, body: respBody } = await createWooOrder(base, auth, wcPayload);
